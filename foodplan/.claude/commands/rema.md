Use Chrome DevTools MCP to scrape food deals from https://etilbudsavis.dk/REMA-1000 and save the results as JSON to @sale/ with the date in the filename.
...
This script will:
- Prompt you for the publication ID (get from URL: `?publication=XXXXX`)
- Prompt for date range and week number
- Extract all deals via API
- Parse quantities and categorize products
- Save to `sale/YYYY-MM-DD/rema_deals.json`
- Complete in ~30 seconds

**How to get publication ID**:
1. Visit https://etilbudsavis.dk/REMA-1000
2. Click the latest magazine
3. Copy the ID from URL after `?publication=`
...
1. Navigate to https://etilbudsavis.dk/REMA-1000
2. The website should open with the newest magazine automatically
3. If not, look for and click the button that says "REMA 1000 uge <newest-week-number>" (e.g., "REMA 1000 uge 46")
...
- **is_app_price**: Boolean - true if product requires Rema 1000 app for special price
...
Save the JSON output to: `sale/[YYYY-MM-DD]/rema_deals.json`

Where [YYYY-MM-DD] is today's date (e.g., sale/2025-11-02/rema_deals.json)
