-- Migration: Fix User Profiles for Family Sharing (v2 - Fixed)
...
-- Author: Claude Code
-- Date: 2025-11-17
-- Fix: Properly handles foreign key constraints before modifying primary key
...
BEGIN
  ...
  IF EXISTS (
    ...
    RAISE NOTICE 'Starting migration of user_profiles table...';

    -- FIRST: Drop all foreign key constraints that reference user_profiles.id
    RAISE NOTICE 'Dropping foreign key constraints...';

    -- Drop foreign key from recipes table if it exists
    IF EXISTS (
      SELECT 1 FROM pg_constraint
      ...
      RAISE NOTICE 'Dropped recipes_user_id_fkey';
    END IF;

    -- Drop foreign key from meal_plans table if it exists
    IF EXISTS (
      SELECT 1 FROM pg_constraint
      ...
      RAISE NOTICE 'Dropped meal_plans_user_id_fkey';
    END IF;

    -- Drop foreign key from shopping_lists table if it exists
    IF EXISTS (
      SELECT 1 FROM pg_constraint
      ...
      RAISE NOTICE 'Dropped shopping_lists_user_id_fkey';
    END IF;

    -- Check if the id column is a foreign key to auth.users
    IF EXISTS (
      SELECT 1 FROM pg_constraint
    ...
    -- Recreate foreign key from recipes to user_profiles
    IF EXISTS (
      ...
      ALTER TABLE recipes
        ...
        ON DELETE CASCADE;
      RAISE NOTICE 'Recreated recipes_user_id_fkey';
    END IF;

    -- Recreate foreign key from meal_plans to user_profiles
    IF EXISTS (
      SELECT 1 FROM information_schema.tables
      ...
      ALTER TABLE meal_plans
        ...
        REFERENCES user_profiles(id)
        ON DELETE CASCADE;
      RAISE NOTICE 'Recreated meal_plans_user_id_fkey';
    END IF;

    -- Recreate foreign key from shopping_lists to user_profiles
    IF EXISTS (
      SELECT 1 FROM information_schema.tables
      ...
      ALTER TABLE shopping_lists
        ...
        REFERENCES user_profiles(id)
        ON DELETE CASCADE;
      RAISE NOTICE 'Recreated shopping_lists_user_id_fkey';
    END IF;
...
CREATE POLICY "Family members can view family meal plans"
  ...
  USING (
    ...
    (user_id IN (
      SELECT id FROM user_profiles WHERE family_id IN (
        SELECT family_id FROM family_members WHERE user_id = auth.uid()
      )
    )) OR
    (family_id IN (
...
-- 7. FIXED: Properly handles foreign key constraints during primary key modification
